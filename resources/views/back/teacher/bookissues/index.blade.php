          ...
          <tr>
            <td style="display: none;">&nbsp;</td>
              <td>@if ($bookissue->isteacher)
                <a href="{{route('teacher.teachers.show',['teacher'=>$bookissue->teacher->id])}}">{{$bookissue->teacher->name}}</a>
              @else
              <a href="{{route('teacher.students.show',['student'=>$bookissue->student->id])}}">{{$bookissue->student->name}}</a>   
              @endif</td>
              <td>{{$bookissue->isteacher? 'yes' : 'no'}}</td>
              <td><a href="{{route('teacher.books.show',['book'=>$bookissue->book->id])}}">{{$bookissue->book ? $bookissue->book->title .'('. $bookissue->book->author .')' : 'no'}}</a></td>
              ...
              <td><a class="btn btn-info" href="{{route('teacher.bookissues.show',['bookissue'=>$bookissue->id])}}"><i class="fa fa-eye"></i></a>
                ...
                <form action="{{route('teacher.bookissues.destroy',['bookissue'=>$bookissue->id])}}" id="{{'delete-form-'.$bookissue->id}}" method="post">
