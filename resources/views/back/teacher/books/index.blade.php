...
<div class="col-md-12 col-sm-12 ">
  <div class="x_panel">
    <div class="x_title">
      <h2><strong class="card-title">All Books</strong>
        </h2>
      <a href="{{route('teacher.books.create')}}" class="btn btn-success pull-right btn-sm">Add New Book</a>
    ...
    <div class="x_content">
      ...
      @foreach ($books as $book)
          <tr>
              ...
              @else
              <img src="{{asset('img/bookdef.jpg')}}" alt="{{$book->title}}" style="width: 90px;">
              @endif</td>
              <td>{{$book->title}}</td>
              <td>{{$book->author}}</td>
              <td>{{$book->rack_no ?? 'N/A'}}</td>
              <td>{{$book->category ? $book->category->name : 'N/A'}}</td>
              <td>{{$book->purchased_at}}</td>
              <td>{{$book->stock ?? 'N/A'}}</td>
              <td>{{$book->stock - $book->issued->count() ?? 'N/A'}}</td>
              <td><a class="btn btn-info" href="{{route('teacher.books.show',['book'=>$book->id])}}"><i class="fa fa-eye"></i></a>
              <a class="btn btn-primary" href="{{route('teacher.books.edit',['book'=>$book->id])}}"><i class="fa fa-edit"></i></a>
                <a class="btn btn-danger"  onclick="document.getElementById('{{'delete-form-'.$book->id}}').submit();"><i class="fa fa-trash"></i></a>
                <form action="{{route('teacher.books.destroy',['book'=>$book->id])}}" id="{{'delete-form-'.$book->id}}" method="post">
