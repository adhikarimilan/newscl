        ...
        <div class="col-md-12 col-sm-12">
          <div class="x_panel">
            ...
            @else 
                Other
            @endif</h6>
            <h6>Class: {{$user->class?$user->class->name:'N/A'}}</h6>
              <h6>Section: {{$user->section?$user->section->name:'N/A'}}</h6>
              <h6>Roll no: {{$user->roll_no}}</h6>
            ...
            <h6>Parent : @foreach ($user->student_parent as $key=>$stdpar)
              @if($key )
                    ,
                  
              @endif
              <a href="#">{{$stdpar->stdparent->name.'('.$stdpar->relation.')'}}</a>
              
            @endforeach
            </h6>
              <a href="{{route('student.profile.edit',['id'=>$user->id])}}" class="btn btn-success">Edit Profile</a>
              </div>
