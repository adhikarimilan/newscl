  ...
  <div class="x_panel">
    ...
    <div class="x_content">
      ...
      @foreach ($teachers as $teacher)
          <tr>
              <td><img src="{{$teacher->avatar ? asset($teacher->avatar) : asset('img/default.jpeg')}}" alt="" style="width:30px;"></td>
              <td>{{$teacher->name }}</td>
              
              @for ($i = 0; $i < $ndays; $i++)
              @php
                  $attendances=$teacher->tattendance;
                  //echo "<br>";
                  //print_r($attendances);
              @endphp
      ...
      @endforeach
       @else
       <td colspan="{{$i+2}}">No data available for this month</td>
       @endif  
      </tbody>
      </table>
    </div>
